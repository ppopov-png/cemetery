...
export class OnnxDepthEstimator {
  ...
  async load(): Promise<void> {
    ...
    this.backend = gpu ? 'webgpu' : 'wasm'; if (!gpu) ort.env.wasm.numThreads = Math.min(4, navigator.hardwareConcurrency || 2)
  ...
  async estimate(source: CanvasImageSource): Promise<DepthFrame> {
    ...
    const inputName = this.session.inputNames[0]; const outputs = await this.session.run({ [inputName]: new ort.Tensor('float32', prepared.data, [1, 3, DEPTH_INPUT_SIZE, DEPTH_INPUT_SIZE]) }); const output = outputs[this.session.outputNames[0]]
