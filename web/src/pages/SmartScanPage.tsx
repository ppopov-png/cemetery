...
import { OnnxGraveDetector } from '../ai/grave-detection/OnnxGraveDetector'
import { DetectionStabilizer } from '../ai/grave-detection/DetectionStabilizer'
import { TargetSelector } from '../ai/grave-detection/TargetSelector'
import type { DetectionResult, DetectorDiagnostics, ModelStatus } from '../ai/grave-detection/graveDetectionTypes'
...
import { LiveWorldMapPanel } from '../components/LiveWorldMapPanel'
...
export function SmartScanPage({ prepared, onExit }: SmartScanPageProps) {
  ...
  const detectionTimerRef = useRef<number | null>(null); const detectorRef = useRef(new OnnxGraveDetector()); const stabilizerRef = useRef(new DetectionStabilizer()); const selectorRef = useRef(new TargetSelector()); const captureActiveRef = useRef(false)
  ...
  const detectFrame = () => {
    if (captureActiveRef.current || detectorRef.current.getStatus() !== 'READY' || !videoRef.current) return
    const image = readFrame(videoRef.current, canvasRef.current)
    if (!image) { scheduleDetection(detectionTimerRef, detectFrame); return }
    void detectorRef.current.detect({ image }).then((detections) => {
      const selected = selectorRef.current.select(detections); const stable = stabilizerRef.current.update(detections); const candidate = stable ?? selected
      setLastDetection(candidate); setDetectorDiagnostics(detectorRef.current.getDiagnostics(detections.length))
      if (candidate) { setBox(candidate.box); setMessage(stable ? 'Объект определён' : 'Объект найден') }
      if (stable && !captureActiveRef.current) {
        const result = trackerRef.current.lock(stable.box, image)
        if (result.featureCount >= objectTrackingConfig.minFeatures) { captureActiveRef.current = true; setTracking(result); setScanState('CAPTURING'); setMessage('Сканирование началось'); sessionRef.current = createSession(prepared.spatialProvider.mode); sessionRef.current.detectorModelVersion = detectorRef.current.getManifest()?.version; setSession(sessionRef.current); stopSampling(detectionTimerRef); scheduleSample(timerRef, sampleFrame) }
      }
    }).catch((error) => { setModelStatus(detectorRef.current.getStatus()); setDetectorDiagnostics({ ...detectorRef.current.getDiagnostics(), error: error instanceof Error ? error.message : 'Detector inference failed.' }) }).finally(() => { if (!captureActiveRef.current) scheduleDetection(detectionTimerRef, detectFrame) })
  }
  ...
  const startRemoteCapture = () => { const next = createSession(prepared.spatialProvider.mode); next.detectorModelVersion = 'pc-only'; sessionRef.current = next; setSession({ ...next }); captureActiveRef.current = true; setRemoteCaptureActive(true); setScanState('CAPTURING'); setMessage('Медленно снимайте объект с разных сторон'); scheduleSample(timerRef, captureRemoteFrame) }
  const captureRemoteFrame = () => { const video = videoRef.current; if (!video || !captureActiveRef.current) return; void captureUnclassifiedFrame(video).then((captured) => { if (!captured || !sessionRef.current || sessionRef.current.status !== 'active') return; sessionRef.current.frames.push(captured); sessionRef.current.acceptedFrames += 1; sessionRef.current.candidateFrames += 1; setSession({ ...sessionRef.current }); setMessage(`Кадр ${sessionRef.current.frames.length}/${targetFrameCount} сохранён`); if (sessionRef.current.frames.length >= targetFrameCount) { stopSampling(timerRef); captureActiveRef.current = false; setRemoteCaptureActive(false); sessionRef.current.status = 'complete'; sessionRef.current.completedAt = Date.now(); setSession({ ...sessionRef.current }); setScanState('SCAN_COMPLETE'); setMessage('Кадры готовы к отправке на ПК') } else scheduleSample(timerRef, captureRemoteFrame) }) }
  const finish = () => { stopSampling(timerRef); if (sessionRef.current) sessionRef.current.status = 'cancelled'; onExit() }
  const sampleFrame = () => {
    const video = videoRef.current; if (!video || !captureActiveRef.current) return
    ...
    if (result.state === 'LOST') { captureActiveRef.current = false; stabilizerRef.current.reset(); selectorRef.current.reset(); setScanState('OBJECT_LOST'); setMessage('Верните памятник в кадр'); scheduleDetection(detectionTimerRef, detectFrame); return }
    ...
    void captureFrame(video, result, frameQuality, signature, lastDetection?.confidence ?? 0).then((captured) => {
  ...
  return <main className="smart-scan-page">
    ...
    <div className="smart-scan-hud"><header className="scan-topbar"><div><p className="scan-kicker">Cemetery Mapper</p><h1>Smart Scan</h1></div><span className={`spatial-pill spatial-${capabilities.trackingState.toLowerCase()}`}>Spatial tracking: {trackingLabel}</span></header>
      ...
      {!remoteCaptureActive && scanState !== 'SCAN_COMPLETE' && <button className="capture-object-button" type="button" onClick={() => setLiveMapOpen(true)}>LIVE 3D MAPPING</button>}
      ...
      {liveMapOpen && <LiveWorldMapPanel videoRef={videoRef} pose={pose} trackingState={capabilities.trackingState} onClose={() => setLiveMapOpen(false)} onSingleView={() => { setLiveMapOpen(false); setSingleViewOpen(true) }} />}
...
function ScanComplete({ session, onRepeat, onDone, onSend }: { session: ScanSession | null; onRepeat: () => void; onDone: () => void; onSend: () => void }) { return <section className="scan-complete"><h2>СКАН ЗАВЕРШЁН</h2><p>Получено {session?.frames.length ?? 0} хороших ракурсов</p><div className="capture-thumbnails">{session?.frames.map((frame) => <img key={frame.id} src={URL.createObjectURL(frame.image)} alt="Captured view" />)}</div><button type="button" onClick={onSend}>ОТПРАВИТЬ НА ПК</button><button type="button" onClick={onRepeat}>ПОВТОРИТЬ</button><button type="button" onClick={onDone}>ГОТОВО</button></section> }
function SpatialDiagnosticsPanel({ capabilities, diagnostics, tracking, quality, session, modelStatus, detectorDiagnostics }: { capabilities: SmartScanCapabilities; diagnostics: SpatialDiagnostics | null; tracking: ObjectTrackingResult | null; quality: FrameQualityResult | null; session: ScanSession | null; modelStatus: ModelStatus; detectorDiagnostics: DetectorDiagnostics }) { return <details className="xr-diagnostics smart-scan-dev-diagnostics"><summary>Developer diagnostics</summary><div className="diagnostics-content"><span>Scan: {session?.status ?? 'active'}</span><span>Object: {tracking?.state ?? 'INITIALIZING'}</span><span>Object features: {tracking?.featureCount ?? 0}</span><span>Object confidence: {tracking ? `${Math.round(tracking.confidence * 100)}%` : '—'}</span><span>Detector backend: {detectorDiagnostics.backend ?? '—'}</span><span>Model: {modelStatus}</span><span>Inference: {detectorDiagnostics.inferenceMs.toFixed(1)} ms / {detectorDiagnostics.detectorFps.toFixed(1)} FPS</span><span>Detections: {detectorDiagnostics.detections}</span><span>Candidate frames: {session?.candidateFrames ?? 0}</span><span>Accepted frames: {session?.acceptedFrames ?? 0}</span><span>Rejected frames: {session?.rejectedFrames ?? 0}</span><span>Current quality: {quality ? `${Math.round(quality.score * 100)}%` : '—'}</span><span>Last reject: {session?.lastRejectReason ?? quality?.rejectReason ?? '—'}</span><span>View diversity: {session?.viewDiversityScore != null ? `${Math.round(session.viewDiversityScore * 100)}%` : '—'}</span><span>Spatial: {diagnostics?.provider ?? '—'} / {diagnostics?.accuracy?.level ?? '—'}</span><span>Scale: {diagnostics?.scaleStatus ?? 'UNSCALED'}</span><span>GPS quality: {getGpsQuality(capabilities.sensorData.location?.accuracy)}</span></div></details> }
async function captureFrame(video: HTMLVideoElement, tracking: ObjectTrackingResult, quality: FrameQualityResult, signature: ViewSignature, detectorConfidence: number): Promise<CapturedFrame | null> { if (!tracking.box) return null; const canvas = document.createElement('canvas'); const scale = Math.min(1920 / Math.max(video.videoWidth, video.videoHeight), 1); canvas.width = Math.max(1, Math.round(video.videoWidth * scale)); canvas.height = Math.max(1, Math.round(video.videoHeight * scale)); const context = canvas.getContext('2d'); if (!context) return null; context.drawImage(video, 0, 0, canvas.width, canvas.height); const image = await new Promise<Blob | null>((resolve) => canvas.toBlob(resolve, 'image/jpeg', 0.88)); if (!image) return null; return { id: crypto.randomUUID(), timestamp: Date.now(), image, width: canvas.width, height: canvas.height, quality, graveBox: tracking.box, detectorConfidence, trackerConfidence: tracking.confidence, ocrCandidateScore: quality.score, relativeView: classifyView(signature), viewSignature: signature } }
async function captureUnclassifiedFrame(video: HTMLVideoElement): Promise<CapturedFrame | null> { const canvas = document.createElement('canvas'); const scale = Math.min(1920 / Math.max(video.videoWidth, video.videoHeight), 1); canvas.width = Math.max(1, Math.round(video.videoWidth * scale)); canvas.height = Math.max(1, Math.round(video.videoHeight * scale)); const context = canvas.getContext('2d'); if (!context) return null; context.drawImage(video, 0, 0, canvas.width, canvas.height); const image = await new Promise<Blob | null>((resolve) => canvas.toBlob(resolve, 'image/jpeg', 0.9)); if (!image) return null; const graveBox = { x: 0, y: 0, width: 1, height: 1 }; const quality: FrameQualityResult = { score: 1, sharpness: 1, brightness: 1, exposureQuality: 1, objectVisibility: 1, trackingConfidence: 0, accepted: true }; return { id: crypto.randomUUID(), timestamp: Date.now(), image, width: canvas.width, height: canvas.height, quality, graveBox, detectorConfidence: 0, trackerConfidence: 0, ocrCandidateScore: 0, relativeView: 'OTHER' } }
...
function createSession(mode: 'xr-high' | 'xr-standard' | 'vision' | 'sensor-limited'): ScanSession { return { id: crypto.randomUUID(), startedAt: Date.now(), spatialMode: mode.startsWith('xr-') ? 'ar' : 'limited', frames: [], candidateFrames: 0, acceptedFrames: 0, rejectedFrames: 0, detectorModelVersion: 'unknown', lifecycle: 'SCANNING', status: 'active' } }
...
function scheduleDetection(timerRef: MutableRefObject<number | null>, detect: () => void) { stopSampling(timerRef); timerRef.current = window.setTimeout(detect, 333) }
